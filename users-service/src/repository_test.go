...
func TestUserDB(t *testing.T) {
	...
	//Get
	userResponse, err := repo.Get(userId, testdata.NetworkId)
	expected = nil
	if expected != err {
		t.Errorf("\n...expected = %v\n...obtained = %v", expected, err)
	}
	...
	err = repo.Update(user, testdata.NetworkId)
	expected = nil
	if expected != err {
		t.Errorf("\n...expected = %v\n...obtained = %v", expected, err)
	}
	...
	//GetAll
	users, _, err := repo.GetAll("1", "0", "created_at", "desc", testdata.NetworkId)
	expectedInt := 1
